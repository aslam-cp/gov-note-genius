...
export default function NotingScreen() {
    ...
    (async () => {
      const [caseRes, docsRes, kbRes] = await Promise.all([
        supabase
          .from("noting_cases")
          .select(
            "subject,reference,noting_type,custom_instruction,analysis,noting_text,applied_kb_ids",
          )
          .eq("id", caseId)
          .single(),
        supabase
  ...
  const run = async (refinement: Refinement | null = null) => {
    ...
    try {
      ...
      await supabase
        .from("noting_cases")
        .update({ noting_text: result.noting, status: "drafted" })
        .eq("id", caseId);
  ...
  return (
    <div className="space-y-6">
      <div>
        <Link
          to={`/case/${caseId}/analysis` as string}
        ...
        {meta && (
          <p className="text-sm text-muted-foreground">
            <strong>Subject:</strong> {meta.subject} &nbsp;·&nbsp; <strong>Reference:</strong>{" "}
            {meta.reference}
          </p>
        )}
      </div>
        ...
        <>
          ...
          <div className="paper p-4 space-y-3">
            ...
            <div className="flex flex-wrap items-center gap-2 pt-2 border-t border-border">
              ...
              <RefineBtn
                onClick={() => run("stronger_rules")}
                loading={loading}
                icon={<BookText className="h-3.5 w-3.5" />}
              >
                Stronger Rule-Based Reasoning
              </RefineBtn>
            </div>
          </div>
        </>
