...
export const analyzeCase = createServerFn({ method: "POST" })
  .inputValidator((data: { documents: DocBlob[]; extractedText?: string; useRuleLibrary?: boolean }) => data)
  .handler(async ({ data }) => {
    const useLib = data.useRuleLibrary === true;
    const library = useLib ? await fetchRuleLibrary() : { summary: "", docs: [] as DocBlob[] };
...
export const generateNoting = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const libBlock = useLib
      ? `\n\n=== KERALA RULE LIBRARY (officer-curated) ===\n${library.summary}`
      : `\n\n=== KERALA RULE LIBRARY ===\nNot consulted for this draft. Do NOT cite specific rule numbers, GO numbers, KFC articles, Stores Purchase Manual paragraphs or KPWD Manual paragraphs unless they appear in the case documents.`;

    const ruleRule = useLib
      ? `- When citing a rule, KFC article, Stores Purchase Manual paragraph, KPWD Manual paragraph, GO or circular, only cite items that actually appear in the case documents or in the Rule Library above. If unsure, do not cite a number — phrase it generally.`
      : `- Do NOT cite any specific KFC article number, Stores Purchase Manual paragraph, KPWD Manual paragraph, GO number or circular number unless it appears verbatim in the case documents. Phrase rule references generally (e.g. "the relevant provisions of the Kerala Financial Code").`;

    const prompt = `Using the analysis and the ATTACHED case documents${useLib ? " and the Kerala Rule Library" : ""}, draft an official Government of Kerala file noting in Sachivalayam style.\n\nThe documents may contain Malayalam text. Read it directly, translate it to formal administrative English, and write the entire noting in English.\n\nNoting type guidance: ${guidance}\n${data.customInstruction ? `\nOfficer's custom instruction: ${data.customInstruction}` : ""}\n\nStructured analysis (JSON):\n${JSON.stringify(data.analysis, null, 2)}${libBlock}\n\nRules for drafting:\n- Ground every statement in the analysis, the case documents${useLib ? ", or the Rule Library" : ""}. Do NOT invent file numbers, names, dates, amounts, sanctions, KFC articles, Stores Purchase Manual paragraphs, KPWD Manual paragraphs, or GOs/circulars.\n${ruleRule}\n- If a fact is not on record, either omit it or say "the record does not disclose…".\n- Begin with a numbered paragraph 1 stating the matter examined.\n- Use Kerala Sachivalayam administrative phrasing such as "The matter has been examined.", "On perusal of the records placed in the file...", "It is seen that...", "In the circumstances, the proposal may be considered...".\n- Number paragraphs (1., 2., 3., ...).\n- End with the appropriate submission line and a signature block placeholder line: "(Section Officer)" / "(Under Secretary to Government)" as appropriate.\n- Output ONLY the noting text in English, no preamble, no markdown headings.${refine}`;
